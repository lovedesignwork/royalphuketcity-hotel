    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">Spa Booking</h2>
            <p className="text-xs text-gray-500 mt-0.5">Royal Wellness Spa · Phuket</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)]">
          <div className="space-y-6">
            <div className="bg-[#6B8E7B]/5 border border-[#6B8E7B]/20 rounded-xl p-4">
              <p className="text-xs font-semibold text-[#6B8E7B] uppercase tracking-wider mb-3">Booking Details</p>
            ...
            <div>
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Guest Information</p>
              <div className="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                  ...
                  <a href={`mailto:${reservation.email}`} className="text-[#6B8E7B] hover:underline block">
                    {reservation.email}
                  </a>
                </div>
                <div>
                  <span className="text-gray-500">Phone</span>
                  <a href={`tel:${reservation.phone}`} className="text-[#6B8E7B] hover:underline block">
                    {reservation.phone}
                  </a>
                </div>
                <div>
            ...
            <div className="border-t border-gray-200 pt-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <a
                    href={`mailto:${reservation.email}?subject=Your booking at Royal Wellness Spa`}
                    className="block w-full px-4 py-2 bg-[#6B8E7B] text-white rounded-lg hover:bg-[#5a7a69] transition-colors text-center text-sm"
                  >
                    Reply via Email
                  </a>
                </div>
        ...
        <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="px-4 py-2 bg-[#6B8E7B] text-white rounded-lg hover:bg-[#5a7a69] disabled:opacity-50 text-sm"
          >
...
export default function SpaReservationsPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/spa/reservations?${params}`);
      if (res.ok) {
  ...
  async function handleUpdate(id: string, status: string, notes: string) {
    const res = await fetch("/api/admin/spa/reservations", {
      method: "PATCH",
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/spa/reservations?id=${id}`, { method: "DELETE" });
    if (res.ok) fetchData();
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-[#6B8E7B] uppercase tracking-[0.3em] mb-1">
            <span className="w-1.5 h-1.5 bg-[#6B8E7B] rounded-full" /> Royal Wellness Spa
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Reservations</h1>
          <p className="text-gray-500 mt-1">Bookings from royalwellnessspaphuket.com</p>
        </div>
        {newCount > 0 && (
          <div className="flex items-center gap-2 px-4 py-2 bg-red-50 rounded-lg">
            <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
            <span className="text-sm text-red-700 font-medium">
              {newCount} new {newCount === 1 ? "booking" : "bookings"}
            </span>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center h-64">
            <div className="animate-spin w-8 h-8 border-2 border-[#6B8E7B] border-t-transparent rounded-full" />
          </div>
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            ...
            <p className="text-sm">Spa bookings from royalwellnessspaphuket.com will appear here</p>
          </div>
        ) : (
          <>
              ...
              <tbody className="divide-y divide-gray-100">
                {reservations.map((r) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <p className="text-gray-900 text-sm max-w-xs truncate">{r.service || "-"}</p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          href={`mailto:${r.email}?subject=Your booking at Royal Wellness Spa`}
                          className="px-3 py-1.5 bg-[#6B8E7B]/10 hover:bg-[#6B8E7B]/20 rounded-lg text-xs font-medium text-[#6B8E7B]"
                        >
                          Reply
                        </a>
                        <button
