          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <a href={`mailto:${inquiry.email}`} className="text-[#6B8E7B] hover:underline block">
                  {inquiry.email}
                </a>
              </div>
              <div>
            ...
            <div className="border-t border-gray-200 pt-6">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <a
                    href={`mailto:${inquiry.email}?subject=Re: ${inquiry.subject || 'Your inquiry at Royal Wellness Spa'}`}
...
export default function SpaInquiriesPage() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/spa/inquiries?${params}`);
      if (res.ok) {
  ...
  async function handleUpdate(id: string, status: string, notes: string) {
    const res = await fetch("/api/admin/spa/inquiries", {
      method: "PATCH",
  ...
  async function handleDelete(id: string) {
    ...
    const res = await fetch(`/api/admin/spa/inquiries?id=${id}`, { method: "DELETE" });
    if (res.ok) fetchData();
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Contact messages from royalwellnessspaphuket.com</p>
        </div>
        {newCount > 0 && (
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-gray-500">
            ...
            <p className="text-sm">Messages from royalwellnessspaphuket.com will appear here</p>
          </div>
        ) : (
          <>
                ...
                {inquiries.map((i) => (
                  <tr
                    ...
                    className={`hover:bg-gray-50 cursor-pointer transition-colors ${
                      i.status === "new" || !i.status ? "bg-blue-50/30" : ""
                    }`}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-gray-900">{i.subject || "-"}</td>
                    <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">
                      {i.message.substring(0, 60)}{i.message.length > 60 ? "..." : ""}
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">
                      {new Date(i.created_at).toLocaleDateString()}
                      <br />
                      <span className="text-xs">{new Date(i.created_at).toLocaleTimeString()}</span>
                    </td>
                    <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        ...
                        <a
                          href={`mailto:${i.email}?subject=Re: ${i.subject || 'Your inquiry at Royal Wellness Spa'}`}
