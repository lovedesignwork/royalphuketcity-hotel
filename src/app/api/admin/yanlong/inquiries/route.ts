...
/**
 * Yan Long inquiries — CRUD for the yanlong_contact_submissions table.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      .from("yanlong_contact_submissions")
      .select("*", { count: "exact" })
    ...
    if (error) {
      console.error("yanlong_contact GET:", error);
      return NextResponse.json({ submissions: [], total: 0 });
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("yanlong_contact PATCH:", error);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error("yanlong_contact DELETE:", error);
